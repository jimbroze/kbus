...
class MessageHandlerFactoryStoreTest {
    ...
    fun `reports no message types when nothing is registered`() {
        val store = MessageHandlerFactoryStore<com.jimbroze.kbus.api.messages.event.Event>()
    ...
    fun `keeps the handlers of different message types apart`() {
        val store = MessageHandlerFactoryStore<com.jimbroze.kbus.api.messages.command.Command<*>>()
