 ...
 *
 * [seen] is the tombstone set — the union of pending and consumed ids — and is never pruned here; a
 * real store must prune it by time, per [InboxStore.markConsumed]'s retention contract.
 */
class InMemoryInboxStore : InboxStore {
    ...
    override suspend fun save(envelopes: List<EventEnvelope>) {
        mutex.withLock { envelopes.forEach { if (seen.add(it.id)) pending.add(it) } }
    }
