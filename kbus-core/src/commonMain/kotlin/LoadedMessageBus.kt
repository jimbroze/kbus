...
open class LoadedMessageBus(handlerLocator: HandlerLocator, middleware: List<Middleware>) :
    MessageBus(handlerLocator, middleware) {
    suspend fun <TCommand : Command, TReturn : Any?, TFailure : FailureReason> execute(
        loadedCommand:
            LoadedCommand<
                TCommand,
                out CommandHandler<TCommand, TReturn, TFailure>,
                TReturn,
                TFailure,
            >
    ...
    suspend fun <TQuery : Query, TReturn : Any?, TFailure : FailureReason> fetch(
        loadedQuery:
            LoadedQuery<TQuery, out QueryHandler<TQuery, TReturn, TFailure>, TReturn, TFailure>
